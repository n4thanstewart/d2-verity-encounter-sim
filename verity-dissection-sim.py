...
def setupEncounter(challengeType):
    callout = generateInnerCallout()
    if challengeType=='' or challengeType is None:
...
def main():
    if cr:
        challengeType=input("\nChallenge type [Hit Enter: Normal] [1: One Double] [2: All Doubles]: ")
    else:
        challengeType=None
    
    callout, leftStatue, middleStatue, rightStatue = setupEncounter(challengeType)
    ...
    timer_thread = threading.Thread(target=startTimer, args=(90,)) # 1 minute 30 seconds
